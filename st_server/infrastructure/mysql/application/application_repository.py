...
from st_server.infrastructure.mysql.application.application import ApplicationDbModel
...
class ApplicationRepository(Repository):
        ...
        with self._session as session:
            ...
            for attr in inspect(ApplicationDbModel).attrs:
                ...
                if kwargs and attr.key in kwargs:
                    ...
                    query = query.filter(
                        FILTER_OPERATOR_MAPPER[op](ApplicationDbModel, attr.key, val)
                    )
    ...
    def find_one(
        ...
        """
        with self._session as session:
            query = session.query(ApplicationDbModel).filter(ApplicationDbModel.id == id)
            ...
            return (
                Application.from_dict(application.to_dict(exclude=exclude)) if application else None
            )
